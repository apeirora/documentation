...
**1. API First**
    ...
    Design clean, versioned APIs; avoid passing unstructured data.

**2. Declarative API**

    Allow declaring desired states; use asynchronous responses with a standard pattern.

**3. Reconciling controllers**

    Continuously push the environment towards the desired state.

**4. Metadata in Data Plane**

    Make all metadata queryable via API with a consistent model.
