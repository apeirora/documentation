...
To be able to demonstrate the power of the Apeiro toolkit, we propose ways how to assemble a working environment out of the individual components.
We call a functioning deployment of Apeiro and other components an Apeiro "Showroom".
...
The Apeiro toolkit covers functionality on different layers, and depending on your specific needs, you may choose to focus on different parts of the toolkit, which will also influence the complexity of the required setup.

The different setups are described in the following chapters.

## Setup as Application and Service Platform

When using Apeiro to establish a cloud platform that should host applications and services, the underlying infrastructure might be secondary and not in focus when setting up a showroom environment.
...
At any time in the process, the infrastructure layer could be replaced with an infrastructure provided and managed by the respective Apeiro components (see subsequent sections).

### Application Hosting

The basic Apeiro components for hosting applications and services are Gardener, Platform Mesh, OpenMFP, and OpenFGA.
...
Installation instructions are available for [OCM](https://ocm.software/docs/getting-started/installation/).
...
In scenarios where Apeiro is used to manage the full cloud stack from hardware to services, additional investments into [hardware](/docs/showroom/hardware-recommendations/index.md) and setup of the bare metal operating system layer is required.
