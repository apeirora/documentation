...
Designing and composing cloud-native workloads into comprehensive SaaS application often requires a microservice-based software delivery and deployment framework (or platform) that covers the LCM aspects holistically. Apeiro project [Konfidence](konfidence/index.md) not only aims to enhance the efficiency for SaaS product teams. By enabling development teams to deploy their daily work directly into production (albeit, behind a feature flag or version vector), their _konfidence_ will change the entire engineering culture.
