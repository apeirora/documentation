...
This is in direct support of the [Managed Service Provider pattern](../services/managed-service-provider-pattern), as APIExports can be reconciled by a remote component on a service provider's environment.
...
Driven by its API-first mindset, kcp enables consumers a wide array of usage patterns; while users can start interacting with kcp via <Project>OpenMFP</Project>, they can (but don't have to) transition to more technical tools like `kubectl`, IaC (Infrastructure as Code) or a GitOps approach.
