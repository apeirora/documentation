...
- Control encryption keys across several platforms and heterogeneous infrastructure providers at once
...
Not all encryption keys are stored directly in an HSM. Instead, encryption keys usually form a hierarchy of keys, also called keychain, where an encryption key is wrapped (i.e. encrypted) by a higher-level encryption key. This process is repeated along the keychain until an encryption key is wrapped by a key that is protected by an HSM. This mechanism allows to store a small number of encryption keys in the HSM while other encryption keys along the keychain can be stored in a regular storage as a wrapped key (i.e. encrypted).
...
OpenKCM, takes this concept one step further and applies it to multiple clusters on the same platform or even across heterogeneous infrastructure providers. It thereby offers a consistent interface for managing encryption of data at rest.
...
The Apeiro Key Management Service consists of several logical components:
